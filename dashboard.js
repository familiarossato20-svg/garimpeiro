function gerarDashboardHTML(oportunidades, stats, historico) {
  ...
  const topMargem = oportunidades.length > 0 
    ? `R$ ${oportunidades[0].margemLiquida.toLocaleString('pt-BR')}` 
    : 'N/A';

  const mediaMargens = oportunidades.length > 0
  ...
  const rows = oportunidades.map((op, i) => {
    ...
    return `
      <tr class="row" onclick="toggleDetalhes(${i})">
        ...
        <td class="fipe">R$ ${op.fipe.toLocaleString('pt-BR')}</td>
        <td class="margem">R$ ${op.margemLiquida.toLocaleString('pt-BR')}</td>
        <td class="percent">${op.percentualAbaixoFipe}%</td>
        <td><span class="badge" style="background:${badge};color:${badgeText}">${op.fonte}</span></td>
        <td class="actions">
          <a href="${op.link}" target="_blank" class="btn-link">Ver anúncio</a>
        </td>
      </tr>
      <tr class="detalhes" id="det-${i}" style="display:none">
        <td colspan="8">
          <div class="detalhes-content">
            ...
            <span>📊 FIPE: ${op.codigoFipe || 'N/A'}</span>
            <span>🏆 Score: ${op.score}</span>
            <span>💰 Margem bruta: R$ ${op.margemBruta.toLocaleString('pt-BR')}</span>
            <span>🔧 Custo prep.: R$ 1.500</span>
          </div>
        </td>
      </tr>`;
  }).join('');
...
<body>
  <div class="header">
    ...
    <div class="subtitle">${hoje} — ${oportunidades.length} oportunidades com margem ≥ R$ 8.000</div>
    <div class="controls">
  ...
  <div class="kpis">
    ...
    <div class="kpi">
      ...
      <div class="kpi-value amber">${stats.totalAnalisados}</div>
    </div>
    <div class="kpi">
  ...
  <div class="filtros-ativos">
    ...
    <span class="filtro-tag">Margem ≥ R$ 8.000</span>
